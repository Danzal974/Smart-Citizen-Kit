...
boolean SCKBase::checkText(char* text, char *text1)
{
  ...
  if (check == limit) {
    ...
    if (j < buffer_length2) text1[j] = 0x00;
    return true;
  }
  else return false;
}
...
boolean SCKBase::sendCommand(const __FlashStringHelper *command, boolean isMultipartCommand = false, const char *expectedResponse = "AOK")
{
...
void SCKBase::setDebugState(boolean debugON)
{
  _debugON = debugON;
}

boolean SCKBase::getDebugState()
{
  return _debugON;
}
